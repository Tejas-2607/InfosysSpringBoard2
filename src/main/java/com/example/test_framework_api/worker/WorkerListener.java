...
//                     request.getTestSuiteId(),
//                     run,
//                     parallelThreads).whenComplete((result, ex) -> {
//                         if (ex != null) {
//                             log.error("Parallel execution failed: {}", ex.getMessage());
//                             run.setStatus(TestStatus.FAILED);
//                         } else {
...
// log.warn("MAX RETRIES EXCEEDED – marking FAILED and sending to DLQ");
// TestRun testRun =
// testRunRepository.findById(request.getTestId()).orElse(null);
// if (testRun != null) {
// updateTestRun(testRun, TestStatus.FAILED);
// saveResult(testRun, TestStatus.FAILED, duration, recovery.getRetryCount());
// }
...
// @RabbitListener(queues = "elementTestQueue", containerFactory =
// "rabbitListenerContainerFactory")
// public void handleElementTest(Map<String, Object> payload) {
// String url = (String) payload.get("url");
// String elementId = (String) payload.get("elementId");
// String action = (String) payload.get("action");
// List<Map<String, Object>> actionsList = (List<Map<String, Object>>)
// payload.get("actions");
// String expectedResult = (String) payload.get("expectedResult");
// Object testRunIdObj = payload.get("testRunId");
// Long testRunId = (testRunIdObj instanceof Number) ? ((Number)
// testRunIdObj).longValue()
// : Long.valueOf(testRunIdObj.toString());
...
// if (testRun != null) {
// updateTestRun(testRun, TestStatus.PASSED);
// saveResult(testRun, TestStatus.PASSED, System.currentTimeMillis() -
// startTime, 0);
// }
// } catch (Exception e) {
// long duration = System.currentTimeMillis() - startTime;
...
// public void handleSuiteExecution(TestCaseExecutionRequest request) {
// log.info("Received suite execution request: Suite {}, Run {}, Threads {}",
// request.getTestSuiteId(), request.getTestRunId(),
...
// List<TestCase> cases =
// caseRepository.findByTestSuiteId(request.getTestSuiteId());
// int executed = 0;
// int passed = 0;
// int failed = 0;
...
// try {
// log.info("Executing test case: {} - {}", tc.getTestCaseId(),
// tc.getTestName());
// testExecutor.executeTestCase(tc, run);
...
// } catch (Exception e) {
// failed++;
// log.error("✗ EXCEPTION in test case {}: {}", tc.getTestCaseId(),
// e.getMessage());
...
// try {
// suiteService.updateSuiteStatus(request.getTestSuiteId());
// log.info("Sequential suite execution complete: {} executed, {} passed, {}
// failed",
...
// log.info("Suite {} execution finished, system remains active",
// request.getTestSuiteId());
// }
...
// private void saveResult(TestRun tr, TestStatus status, long duration, int
// retryCount) {
// TestResult r = new TestResult();
// r.setTestName(tr.getName());
// r.setStatus(status);
...
// r.setRetryCount(retryCount);
// testResultService.saveTestResult(r);
// log.debug("Saved TestResult for TestRun ID: {} | Status: {} | Duration: {}ms
// | Retries: {}",
// tr.getId(), status, duration, retryCount);
// }
// }