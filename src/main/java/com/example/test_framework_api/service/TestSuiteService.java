...
/**
 * ENHANCED: Edge case handling with appropriate async executors
 * - Sequential fallback (no async overhead)
 * - Empty suite validation
 * - Mixed failure aggregation with accurate pass rates
 */
@Service
@RequiredArgsConstructor
@Slf4j
public class TestSuiteService {
    ...
    /**
     * ENHANCED: Execute suite with edge case handling and appropriate async executors.
     * 
     * Edge Cases:
     * 1. threads=1 → Uses generalExecutor for sequential (no parallelism)
     * 2. threads<1 or invalid → Logged warning, defaults to 1
     * 3. Empty suite → Immediate return with COMPLETE status
     * 4. Mixed failures → Accurate aggregation with pass rate
     * 
     * @param suiteId Suite to execute
     * @param run TestRun for result tracking
    ...
    public CompletableFuture<Void> executeSuiteParallel(Long suiteId, TestRun run, int parallelThreads) {
        ...
        // EDGE CASE 2: Validation (defense in depth)
        ...
        // EDGE CASE 1: Sequential fallback (threads=1)
        if (parallelThreads == 1) {
            log.info("Executing suite {} in SEQUENTIAL mode (no parallelism)", suiteId);
            return executeSequential(enabledCases, run, suiteId);
        }

        // Parallel execution (threads > 1)
        log.info("Executing suite {} in PARALLEL mode with {} threads", suiteId, parallelThreads);
        return executeParallel(enabledCases, run, suiteId);
    }

    /**
     * EDGE CASE 1: Sequential execution (no async overhead).
     * Uses generalExecutor (already async for non-blocking controller).
     */
    private CompletableFuture<Void> executeSequential(List<TestCase> cases, TestRun run, Long suiteId) {
        ...
        for (TestCase tc : cases) {
            try {
                log.debug("Sequential execution: {}", tc.getTestCaseId());
                testExecutor.executeTestCase(tc, run);
                
                // Check result (assumes synchronous save in executeTestCase)
                ...
                log.error("Sequential execution failed for {}: {}", tc.getTestCaseId(), e.getMessage());
                failed++;
            }
        }
        
        // EDGE CASE 4: Mixed failure aggregation
        log.info("Sequential execution complete: {} passed, {} failed (rate: {:.2f}%)", 
    ...
    /**
     * Parallel execution with type-appropriate executors.
     * UI tests → uiTestExecutor (limited concurrency)
     * API tests → apiTestExecutor (higher concurrency)
     */
    private CompletableFuture<Void> executeParallel(List<TestCase> cases, TestRun run, Long suiteId) {
        ...
        return allOf.whenComplete((result, ex) -> {
            ...
            // EDGE CASE 4: Update suite status after all tests complete
            updateSuiteStatus(suiteId);
        });
    }

    /**
     * ENHANCED: Update suite status with accurate mixed failure aggregation.
     * 
     * EDGE CASE 4: Handles partial failures correctly:
     * - All passed → PASSED
     * - Some passed → COMPLETED (partial success)
     * - None passed → FAILED
     */
    public void updateSuiteStatus(Long suiteId) {
        TestSuite suite = getSuiteById(suiteId);
        ...
        if (results.isEmpty()) {
            ...
            // EDGE CASE 4: Mixed failure logic
